...
// THIS ONE ALSO WORKS (with all members):
namespace App\Http\Controllers;
...
class JoinRequestController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Get the companies where the authenticated user is a boss
        $bossCompanies = $user->companies()->wherePivot('is_boss', 1)->pluck('companies.id')->toArray();

        // Get join requests for those companies
        $requests = JoinRequest::whereIn('company_id', $bossCompanies)->with('user')->get();
        $requestCount = $requests->count();

        // Get users from the companies where the authenticated user is a boss
        $users = User::whereHas('companies', function ($query) use ($bossCompanies) {
            $query->whereIn('companies.id', $bossCompanies);
        })->with('companies')->get();
        $userCount = $users->count();

        return view('boss.all-members', compact('requests', 'requestCount', 'users', 'userCount', 'bossCompanies'));
    }
